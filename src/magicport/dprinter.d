...
class DPrinter : Visitor
{
    ...
    {
        if (auto de = cast(DeclarationExpr)ast.xinit)
        {
            if (auto vd = cast(VarDeclaration)de.d)
            {
                if (auto bt = cast(BasicType)vd.type)
                {
                    if (bt.id == "size_t" || bt.id == "int")
                    {
                        if (auto le = cast(CmpExpr)ast.cond)
                        {
                            if (le.op == "<")
                            {
                                if (auto ie = cast(IdentExpr)le.e1)
                                {
                                    if (ie.id == vd.id)
                                    {
                                        if (auto pe = cast(PostExpr)ast.inc)
                                        {
                                            if (pe.op == "++")
                                            {
                                                if (auto ie2 = cast(IdentExpr)pe.e)
                                                {
                                                    if (ie2.id == vd.id)
                                                    {
                                                        print("foreach (");
