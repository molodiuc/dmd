...
void LibMach::addObject(const char *module_name, void *buf, size_t buflen)
{
    ...
    {   /* Library file.
         ...
         */
        unsigned nsymbols = Port::sgetl(symtab) / 8;
        ...
        {
            unsigned soff = Port::sgetl(symtab + 4 + i * 8);
            char *name = s + soff;
            ...
            unsigned moff = Port::sgetl(symtab + 4 + i * 8 + 4);
            //printf("symtab[%d] moff = x%x  x%x, name = %s\n", i, moff, moff + sizeof(MachLibHeader), name);
...
//      moffset += 4;
    ...
    Port::sputl(objsymbols.dim * 8, buf);
    libbuf->write(buf, 4);
